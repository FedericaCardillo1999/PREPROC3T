...
gauss_iter_bounds = [
    ...
    (0.01, 1*max_eccentricity),                             # PRF size bounds (changed from 0.1 to 0.05)
...
def save_params(model, model_name):
    if rois.__len__()==1:
        pkl_file = opj(f'{MAIN_PATH}/pRFM/{subject}/ses-02/{denoising}/', f'roi-{rois[0]}_model-{atlas}-{model_name}-{depth}_desc-prf_params.pkl')       
    else:
        pkl_file = opj(f'{MAIN_PATH}/pRFM/{subject}/ses-02/{denoising}/', f'model-{atlas}-{model_name}-{depth}_desc-prf_params.pkl')                        
